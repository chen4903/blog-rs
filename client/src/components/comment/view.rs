...
use crate::{
    ...
    fetch,
    models::comment::Comment, app::Route,
};
                                            ...
                                            <article class="card" key={i.id.unwrap()}>
                                                <header>
                                                    ...
                                                    if is_admin || (user_id.is_some() && user_id.unwrap() == i.user.as_ref().unwrap().id) {
                                                        <button class="error" onclick={jump(navigator.clone(),
                                                        Route::DeleteComment {comment_id: i.id.unwrap()})}>{"删除"}
                                                        </button>
                                                    } 
